...
use na::{Vector3,Vector6,Matrix4,SMatrix,U2,U4,U6,U9,RowVector2,Vector4,SVector,DVector,Matrix,DMatrix,Dynamic,VecStorage};
use std::boxed::Box;
...
fn norm(residuals: &DVector<Float>,loss_function :&Box<dyn LossFunction>, weights_vec: &mut DVector<Float>) ->() {
    for i in 0..residuals.len() {
        let res = residuals[i];
        weights_vec[i] = (loss_function.second_derivative_at_current(res) * res).abs().sqrt(); // Squareroot here is technically too hardcoded
    }
