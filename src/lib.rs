...
use self::pyramid::octave::Octave;
...
pub fn feature_vectors_from_octave(octave: &Octave, sigma_level: usize) -> Vec<FeatureVector> {
    let x_step = 1;
    let y_step = 1;
    ...
    descriptors.iter().map(|x| FeatureVector::new(x)).collect::<Vec<FeatureVector>>()
}
