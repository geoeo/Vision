...
impl<F: SupersetOf<Float>, C : Camera<Float> + 'static, L: Landmark<F, LANDMARK_PARAM_SIZE> + Copy + Clone + Send + Sync  + 'static, const LANDMARK_PARAM_SIZE: usize>  Solver<F,C,L,LANDMARK_PARAM_SIZE> where F: float::Float + Scalar + RealField  {
     ...
     * */
    fn get_estimated_features(state: &State<F,L,LANDMARK_PARAM_SIZE>, cameras: &Vec<&C>,observed_features: &DVector<F>, estimated_features: &mut DVector<F>) 
        -> () {
        ...
        for i in 0..n_cams {
            ...
            for j in 0..n_points {
                ...
                if !(observed_features[feat_id] == convert(state_linearizer::NO_FEATURE_FLAG)|| observed_features[feat_id+1] == convert(state_linearizer::NO_FEATURE_FLAG) || estimated_feature.is_none()){
    ...
    fn compute_jacobian_wrt_object_points(camera: &C, state: &State<F,L,LANDMARK_PARAM_SIZE>, cam_idx: usize, point_idx: usize, i: usize, j: usize, jacobian: &mut DMatrix<F>) -> (){
        ...
        jacobian.fixed_view_mut::<2,LANDMARK_PARAM_SIZE>(i,j).copy_from(&local_jacobian.fixed_view::<2,LANDMARK_PARAM_SIZE>(0,0));
    }
