...
pub struct SFMConfig<C, C2, Feat: Feature> {
    ...
    camera_map_low_precision: HashMap<usize, C2>,
...
impl<C, C2, Feat: Feature> SFMConfig<C,C2, Feat> {
    ...
    pub fn new(root: usize, paths: Vec<Vec<usize>>, camera_map: HashMap<usize, C>, camera_map_low_precision: HashMap<usize, C2>, matches: Vec<Vec<Vec<Match<Feat>>>>) -> SFMConfig<C,C2,Feat> {
        for key in camera_map.keys() {
            assert!(camera_map_low_precision.contains_key(key));
        }

        for key in camera_map_low_precision.keys() {
            assert!(camera_map.contains_key(key));
        }
        SFMConfig{root, paths, camera_map, camera_map_low_precision, matches}
    }
    ...
    pub fn camera_map_low_precision(&self) -> &HashMap<usize, C2> { &self.camera_map_low_precision }
    ...
    pub fn compute_unqiue_ids_cameras_lowp_root_first(&self) -> (Vec<usize>, Vec<&C2>) {
        let keys_sorted = self.get_sorted_camera_keys();
        let cameras_sorted = keys_sorted.iter().map(|id| self.camera_map_low_precision.get(id).expect("compute_unqiue_ids_cameras_lowp_root_first: trying to get invalid camera")).collect::<Vec<&C2>>();
        (keys_sorted,cameras_sorted)
    }

    fn get_sorted_camera_keys(&self) -> Vec<usize> {
        let number_of_keys = self.camera_map_low_precision.keys().len();
