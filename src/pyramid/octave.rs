...
impl Octave {
    ...
    pub fn build_octave(base_image: &Image, s: usize, sigma_initial: Float, runtime_params: &RuntimeParams) -> Octave {
        ...
        let x_gradient = range.clone().map(|x| filter::filter_1d_convolution(&images_borrows,x, GradientDirection::HORIZINTAL, &prewitt_kernel, true)).collect();
        let y_gradient = range.clone().map(|x| filter::filter_1d_convolution(&images_borrows,x, GradientDirection::VERTICAL, &prewitt_kernel, true)).collect();
        ...
        let dog_x_gradient = dog_range.clone().map(|sigma_idx| filter::filter_1d_convolution(&difference_of_gaussians_borrows,sigma_idx, GradientDirection::HORIZINTAL, &prewitt_kernel, true)).collect();
        let dog_y_gradient = dog_range.clone().map(|sigma_idx| filter::filter_1d_convolution(&difference_of_gaussians_borrows,sigma_idx, GradientDirection::VERTICAL, &prewitt_kernel,true )).collect();
        let dog_s_gradient = dog_range.clone().map(|sigma_idx| filter::filter_1d_convolution(&difference_of_gaussians_borrows,sigma_idx, GradientDirection::SIGMA, &prewitt_kernel, true)).collect();
