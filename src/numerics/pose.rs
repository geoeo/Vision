...
pub fn from_matrix<F>(mat: &Matrix4<F>) -> Isometry3<F> where F : Scalar + RealField + Copy {
...
pub fn from_matrix_3x4<F>(mat: &Matrix3x4<F>) -> Isometry3<F> where F : Scalar + RealField + Copy {
    let vec = Vector3::<F>::new(mat[(0,3)],mat[(1,3)],mat[(2,3)]);
    let rot = Matrix3::<F>::new(mat[(0,0)],mat[(0,1)],mat[(0,2)],
                                    mat[(1,0)],mat[(1,1)],mat[(1,2)],
                                    mat[(2,0)],mat[(2,1)],mat[(2,2)]);
...
pub fn se3<F>(t: &Vector3<F>, rotation: &Matrix3<F>) -> Matrix4<F> where F : Scalar + RealField + Copy{
    isometry3(t,rotation).to_homogeneous()
}

pub fn isometry3<F>(t: &Vector3<F>, rotation: &Matrix3<F>) -> Isometry3<F> where F :  Scalar + RealField + Copy{
...
pub fn from_parts<F>(t: &Vector3<F>, quat: &UnitQuaternion<F>) -> Isometry3<F> where F : Scalar + RealField + Copy{
    Isometry3::<F>::from_parts(Translation3::from(*t), *quat)
}

pub fn to_parts<F>(isometry:  &Isometry3::<F>) -> (Vector3<F>, Matrix3<F>) where F : Scalar + RealField + Copy{
 ...
 */
pub fn pose_difference<F>(a: &Isometry3<F>, b:&Isometry3<F>) -> Isometry3<F> where F :Scalar + RealField + Copy {
    b*a.inverse()
}

pub fn decomp<F>(pose:&Isometry3<F>) -> (Vector3<F>,Matrix3<F>) where F :  Scalar + RealField + Copy {
    (pose.translation.vector,pose.rotation.to_rotation_matrix().matrix().into_owned())
}

pub fn apply_pose_deltas_to_point<F>(point: Point3<F>, pose_deltas: &Vec<Isometry3<F>>) -> Vec<Point3<F>> where F : Scalar + RealField + Copy {
...
pub fn error<F>(q_1: &Isometry3<F>,q_2: &Isometry3<F>,p_1: &Isometry3<F>,p_2: &Isometry3<F>) -> Isometry3<F> where F : Scalar + RealField + Copy  {
    ((q_1.inverse()*q_2.inverse())*(p_1.inverse()*p_2.inverse())).inverse()
}

pub fn rsme<F>(data: &Vec<Isometry3<F>>) -> F where F : Scalar + RealField {
    let norm_sum = data.iter().fold(F::zero(), |acc, x| acc + convert(x.translation.vector.norm_squared()));
    let length = data.len() as f64;
    (norm_sum/convert(length)).sqrt()
}

/**
 * 3D Rotations - Kanatani p.35
 */
pub fn optimal_correction_of_rotation<F>(rotation: &Matrix3<F>) -> Matrix3<F> where F : Scalar + RealField + Copy{
