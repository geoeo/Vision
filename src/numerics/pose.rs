...
pub fn se3<F>(t: &Vector3<F>, rotation: &Matrix3<F>) -> Matrix4<F> where F : float::Float + Scalar + NumAssign + SimdRealField + RealField{
    isometry3(t,rotation).to_homogeneous()
}

pub fn isometry3<F>(t: &Vector3<F>, rotation: &Matrix3<F>) -> Isometry3<F> where F : float::Float + Scalar + NumAssign + SimdRealField + RealField{
...
pub fn from_parts<F>(t: &Vector3<F>, quat: &UnitQuaternion<F>) -> Isometry3<F> where F : float::Float + Scalar + NumAssign + SimdRealField + RealField {
    Isometry3::<F>::from_parts(Translation3::from(*t), *quat)
}

pub fn to_parts<F>(isometry:  &Isometry3::<F>) -> (Vector3<F>, Matrix3<F>) where F : float::Float + Scalar + NumAssign + SimdRealField + RealField {
 ...
 */
pub fn pose_difference<F>(a: &Isometry3<F>, b:&Isometry3<F>) -> Isometry3<F> where F : float::Float + Scalar + NumAssign + SimdRealField + RealField {
    b*a.inverse()
}

pub fn decomp<F>(pose:&Isometry3<F>) -> (Vector3<F>,Matrix3<F>) where F : float::Float + Scalar + NumAssign + SimdRealField + RealField {
    (pose.translation.vector,pose.rotation.to_rotation_matrix().matrix().into_owned())
}

pub fn apply_pose_deltas_to_point<F>(point: Point3<F>, pose_deltas: &Vec<Isometry3<F>>) -> Vec<Point3<F>> where F : float::Float + Scalar + NumAssign + SimdRealField + RealField {
...
pub fn error<F>(q_1: &Isometry3<F>,q_2: &Isometry3<F>,p_1: &Isometry3<F>,p_2: &Isometry3<F>) -> Isometry3<F> where F : float::Float + Scalar + NumAssign + SimdRealField + RealField  {
    ((q_1.inverse()*q_2.inverse())*(p_1.inverse()*p_2.inverse())).inverse()
}

pub fn rsme<F>(data: &Vec<Isometry3<F>>) -> F where F : float::Float + Scalar + NumAssign + SimdRealField {
 ...
 */
pub fn optimal_correction_of_rotation<F>(rotation: &Matrix3<F>) -> Matrix3<F> where F : float::Float + Scalar + NumAssign + SimdRealField + ComplexField + RealField{
