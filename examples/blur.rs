...
fn main() {
    ...
    let blur_hor = filter::gaussian_1_d_convolution_horizontal(&frame, 0.0, 5.5);

    let blur_hor_image = blur_hor.to_image();
    blur_hor_image.save(blur_hor_file_out_path).unwrap();

    let blur_2d = filter::gaussian_1_d_convolution_vertical(&blur_hor, 0.0, 5.5);
    let blur_vert = filter::gaussian_1_d_convolution_vertical(&frame, 0.0, 5.5);
